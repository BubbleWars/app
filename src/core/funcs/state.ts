...
export const handleSnapshotContact = (contact: Contact) => {
  ...
  //Bubble-Resource collision
    ...
    snapshotDeferredUpdates.push(() => {
      absorbResource(snapshotBubbles, snapshotResources, b1, r2, STEP_DELTA);
    });
  } else if (b2 && r1) {
    snapshotDeferredUpdates.push(() => {
      absorbResource(snapshotBubbles, snapshotResources, b2, r1, STEP_DELTA);
    });
  }
  ...
  if (n1 && r2) {
    snapshotDeferredUpdates.push(() => {
      nodeAbsorbResource(snapshotNodes, snapshotResources, n1, r2, STEP_DELTA);
    });
  } else if (n2 && r1) {
    snapshotDeferredUpdates.push(() => {
      nodeAbsorbResource(snapshotNodes, snapshotResources, n2, r1, STEP_DELTA);
    });
  }
  ...
  //Node-Bubble collision
    ...
    snapshotDeferredUpdates.push(() => {
      nodeAbsorbBubble(snapshotNodes, snapshotBubbles, n1, b2, STEP_DELTA);
    });
  } else if (n2 && b1) {
    snapshotDeferredUpdates.push(() => {
      nodeAbsorbBubble(snapshotNodes, snapshotBubbles, n2, b1, STEP_DELTA);
    });
  }
