...
export const createBubble = (bubbles: Map<string, Bubble>, world: World, owner: Address, x: number, y: number, mass: number, controllable: boolean): Bubble => {
    ...
    bubble.body.setUserData(generateId());
    bubbles.set(bubble.body.getUserData() as string, bubble);
    return bubble;
}

export const updateBubble = (bubble: Bubble, newMass: number): void => {
    if(newMass <= 0) {
...
export const emitBubble = (bubbles: Map<string, Bubble>, bubble: Bubble, mass: number, direction: Vec2): Bubble => {
    ...
    updateBubble(bubble, newBubbleMass);
...
export const absorbBubble = (bubble: Bubble, absorbedBubble: Bubble, timeElapsed: number): void => {
    ...
    updateBubble(bubble, newBubbleMass);
    updateBubble(absorbedBubble, newAbsorbedBubbleMass);
