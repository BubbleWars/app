...
export const emitResource = (
  ...
  const emittedResourceVelocityMagnitude =
    (bubble.body.getMass() / emittedResource.body.getMass()) * EMISSION_SPEED;
...
export const absorbBubble = (
  ...
  const newBubbleResourceMass =
    getBubbleResourceMass(bubble, ResourceType.Energy) + amountResourceAbsorbed;
...
export const absorbResource = (
  ...
  const futurePositionAbsorbedResource = absorbedResource.body
    .getPosition()
    .clone()
    .add(absorbedResource.body.getLinearVelocity().clone().mul(timeElapsed));
    ...
    if (shouldClash) {
      ...
      if (bubbleResourceMass)
        if (bubbleResourceMass < 0) {
          ...
          const puncture = bubble.punctures.get(puncturePointNormalized);
