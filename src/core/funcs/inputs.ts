...
const ETH_WITHDRAW_FUNCTION_SELECTOR = ethers.keccak256(ethers.toUtf8Bytes("withdrawEther(address,uint256)")).slice(0, 4)
...
export const parseInput = (data: AdvanceData): Input | false => {
    ...
    const type = sender == ETH_PORTAL_ADDRESS ? 
        InputType.Deposit : 
        payloadJSON.type;
...
export const handleInput = async (input: Input | undefined, client:boolean=false): Promise<boolean> => {
...
const handleWithdraw = async (input: Withdraw, client: boolean): Promise<boolean> => {
    ...
    if (user.balance < amount) {
        ...
        const voucher_request =  await fetch(rollup_server + "/voucher", {
            ...
            body: JSON.stringify({
                ...
                payload: ETH_WITHDRAW_FUNCTION_SELECTOR + 
                    ethers.AbiCoder.defaultAbiCoder()
                        .encode(["address","uint256"], [input.sender, amount]),
            }),
        })
...
const handleSpawnPortal = (input: SpawnPortal, client:boolean): boolean => {
    ...
    const userPortal = client ?
        Array.from(snapshotPortals.values()).filter(portal => portal.owner === user.address) :
        Array.from(portals.values()).filter(portal => portal.owner === user.address);
    ...
    user.balance -= amount;
    if(client) {
        const { x, y } = generateSpawnPoint(snapshotWorld, snapshotPortals, snapshotBubbles, obstacles, amount);
        createPortal(snapshotPortals, snapshotWorld, user.address, x, y, amount);
    }else {
        const { x, y } = generateSpawnPoint(world, portals, bubbles, obstacles, amount);
            ...
            try {
                inspect_request = await fetch(rollup_server + "/report", {
                    ...
                    body: JSON.stringify({
                        payload: ethers
                            .hexlify(ethers
                                .toUtf8Bytes(JSON.stringify(currentState)))
                    }),
                });
                ...
                console.error("Error during fetch for InspectType State:", error);
                return false;
            }
            break;
...
export const sendNotice = async (input: Input): Promise<boolean> => {
    const notice_request = await fetch(rollup_server + "/notice", {
        ...
        body: JSON.stringify({
            payload: ethers
                .hexlify(ethers
                    .toUtf8Bytes(JSON.stringify(input)))
        }),
    });
...
export const handlePendingInputs = (input: Input, client: boolean =false): void => {
