...
import { GetNoticesDocument, GetNoticesQuery, Notice } from "../generated-src/graphql";
...
export const useBlockTimestamp = () : number => {
    ...
    const blockNumber = useBlockNumber({chainId: currentChain.id, watch: true});
    useEffect(() => {
        const _ = async () => {
            const block = await getPublicClient({chainId:currentChain.id})
...
export const useMachineTimestamp = (snapshot?: Snapshot, notices: Notice[]) : number => {
...
export const inspectState = async (inspect: Inspect): Promise<Snapshot | undefined> => {
...
export const useInspect = (inspect: Inspect) : {
    loading: boolean,
    error: any,
...
export const useNotices = () : {
    ...
    //Fetch notices
    const { loading, error, data } = useQuery<GetNoticesQuery>(GetNoticesDocument, {
        variables: { cursor },
        pollInterval: 500,
    });
