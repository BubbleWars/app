...
export const Bubble = ({ bubbleId } : { bubbleId: string }) => {
    ...
    useFrame(() => {
       ...
       //console.log("bubble position:", bubble.position)
        const newX = MathUtils.lerp(meshRef.current.position.x, bubble.position.x, 0.05)
        const newY = MathUtils.lerp(meshRef.current.position.y, bubble.position.y, 0.05)
